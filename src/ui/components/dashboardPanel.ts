...
export function buildDashboardActionsPanel(): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('tnt_refresh_data').setLabel('Refresh Data | تحديث البيانات').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tnt_clear_cache').setLabel('Clear Cache | مسح الكاش').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('tnt_restart_bot').setLabel('Restart Bot | إعادة التشغيل').setStyle(ButtonStyle.Danger)
  );
}
